    ...
    if gruppe.temp:
        ...
        for schueler in schueler_liste:
            ...
            if created:
                ...
                if duellant.gruppe == None or duellant.gruppe != gruppe:
                    duellant.gruppe = gruppe
                    duellant.save()
    duellanten = Duellant.objects.filter(gruppe=gruppe)
...
def duell_optionen(req, slug):
    ...
    ret = aufgaben(kategorie.zeile, jg = jg, stufe = stufe, optionen = zaehler.optionen_text)
...
def duell_light_abmelden(req):
    logout(req)
    return redirect('/duell/?abgemeldet=1')
